...
int main(){
    ...
    while(1)
    {
        ...
        for(int i = 1; i < n; i++){
            if(matriz[i][0] > x) x = matriz[i][0];
            if(matriz[i][1] < y) y = matriz[i][1];
            if(matriz[i][2] < u) u = matriz[i][2];
            if(matriz[i][3] > v) v = matriz[i][3];        
        }
        printf("Teste %d\n", teste);
        if(x <= u && y >= v)
        {
